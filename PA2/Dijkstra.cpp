...
float dijkstra(Graph<T>& g, T src) {
    ...
    { 
    	...
    	if(g.vertices[current_vertex]->visited == false){
    		...
    		cost += g.vertices[current_vertex]->distance - g.vertices[g.vertices[current_vertex]->prev]->distance;
    		for(auto it = g.vertices[current_vertex]->edges.begin();it != g.vertices[current_vertex]->edges.end();it++){
    			float alt = g.vertices[current_vertex]->distance + g.get_weight(current_vertex,*it);
